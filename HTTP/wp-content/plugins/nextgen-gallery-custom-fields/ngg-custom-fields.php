...
//{
	...
	define("NGGCF_FIELD_TYPE_DATE", 4);
	...
	//install funcs{
		...
		function nggcf_install($upgrade=false) {
			...
			//collation and charsets added v1.2. Please excuse crappy altering tables/cols code
			$charset_collate = '';
			...
			dbDelta($sql);
			//update tables to correct charset (default latin1 was set when built with old versions)
			$wpdb->query('ALTER TABLE '.$table_name.' CHARSET utf8;');
			$wpdb->query('ALTER TABLE '.$table_name.' MODIFY field_name varchar(255) CHARSET utf8;');
			$wpdb->query('ALTER TABLE '.$table_name.' MODIFY field_type varchar(255) CHARSET utf8;');
			...
			//update tables to correct charset (default latin1 was set when built with old versions)
			$wpdb->query('ALTER TABLE '.$table_name.' CHARSET utf8;');
			$wpdb->query('ALTER TABLE '.$table_name.' MODIFY field_value TEXT CHARSET utf8;');
	...
	//api stuff{
		...
		function nggcf_save_pics($gid, $post) {
			...
			if ( is_array($post["nggcf_fields"]) ) {
				foreach (stripslashes_deep($post["nggcf_fields"]) as $pid=>$fields) {
			...
			if(is_array($post["nggcf_gallery"])) {
				...
				foreach (stripslashes_deep($post["nggcf_gallery"]) as $fid=>$val) {
		...
		function nggcf_hold_field_values($pid) {
			...
			if(!$nggcf_values[$pid]) {
				...
				foreach ((array)$value as $key=>$val) {
					$nggcf_values[$pid][$val->field_name] = $val;
				}
			}
		}
		...
		function nggcf_hold_gallery_field_values($gid) {
			...
			if(!$nggcf_field_values[$gid]) {
				...
				foreach ((array)$value as $key=>$val) {
					$nggcf_field_values[$gid][$val->field_name] = $val->field_value;
				}
			}
		}
	//}
	...
	//admin stuff{
		...
		function nggcf_image_options($type) {
			...
			if($_POST) {
				$conf = stripslashes_deep($_POST["conf"]);
						...
						<tr>
							<td>
								...
								<form id="edit_field_form_<?php echo $val->id ?>" method="POST" action="<?php echo $filepath; ?>" accept-charset="utf-8" style="display:<?php echo $fstyle; ?>;">
									...
									<input type="text" name="conf[field_name]" value="<?php echo esc_attr($val->field_name);  ?>" />
									<input class="button-primary" type="submit" value="<?php _e("Save", "nggcustomfields") ;?>"/>
								</form>
								<span id="edit_field_name_<?php echo $val->id ?>" style="display:<?php echo $nstyle; ?>"><?php echo htmlspecialchars($val->field_name); ?></span>
							</td>
							<td>
								<?php
							...
							<td>
								...
								<?php if($val->field_type == NGGCF_FIELD_TYPE_SELECT) { ?>
									<div id="nggcf-edit-options-<?php echo $val->id; ?>" class="nggcf-edit-options" style="display:<?php echo($_GET["edit"] == $val->id ? "block" : "none"); ?>;">
										<form id="" method="POST" action="<?php echo $filepath; ?>" accept-charset="utf-8" >
											<input type="hidden" name="conf[id]" value="<?php echo $val->id; ?>" />
											<textarea cols="33" name="conf[drop_options]"><?php echo htmlspecialchars($val->drop_options); ?></textarea><br /><i>eg: Some Option, Another Option, Third Option</i>
											<div class="submit">
		...
		function nggcf_new_gallery_form() {
			...
			if($iList) {
				...
				<?php foreach ($iList as $key=>$val) { ?>
					<input type="checkbox" name="conf[nggcf_fields][<?php echo $val->id ?>]" value="1" checked /> <?php echo htmlspecialchars($val->field_name) ?><br />
				<?php } ?>
				</td>
				</tr>
				<?php
		...
		function nggcf_admin_col($gallery_column_key, $pid) {
			...
			$custCol = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."nggcf_fields WHERE field_name = '".$wpdb->escape(htmlspecialchars_decode($gallery_column_key))."'");
			...
			case NGGCF_FIELD_TYPE_INPUT :?>
				<input name="nggcf_fields[<?php echo $pid ?>][<?php echo $custCol->id ?>]" style="width:95%;" value="<?php echo esc_attr($value->field_value); ?>" />
				<?php	break;
			case NGGCF_FIELD_TYPE_TEXTAREA :?>
				<textarea name="nggcf_fields[<?php echo $pid ?>][<?php echo $custCol->id ?>]" style="width:95%;"><?php echo esc_attr($value->field_value); ?></textarea>
				<?php	break;
			case NGGCF_FIELD_TYPE_SELECT :
				...
				<?php
					foreach ((array)$opts as $key=>$val) {
						?>
						<option value="<?php echo trim(esc_attr($val)); ?>"<?php echo trim(esc_attr($val)) == esc_attr($value->field_value) ? " selected" : "" ?>><?php echo trim(esc_attr($val)); ?></option>
						<?php
					}
				?>
				</select>
			...
			/* dependency on JS dropped thanks to new action hook 'ngg_manage_gallery_settings'. See hook/func below for rewrite (keeping code for the hell of it, it's free)
			if(!$ngg_edit_gallery) {
		...
		add_action('ngg_manage_gallery_settings', 'nggcf_admin_gallery_col');
		function nggcf_admin_gallery_col($act_gid) {
			global $wpdb;
			$fields = nggcf_get_field_list(NGGCF_GALLERY, $act_gid);
			
			echo '<tr>';
			echo '<td colspan="4" align="left">';
			echo '<strong>Custom Columns</strong><hr /><input type="hidden" name="nggcf_gallery[ngg_gallery_id]" value="'.$act_gid.'" />';
			
			foreach ((array)$fields as $key=>$val) {
				echo '<tr>';
				echo '<td align="right">'.htmlspecialchars($val->field_name).'</td>';
				echo '<td>';
				
				$value = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."nggcf_field_values WHERE pid = '".$wpdb->escape($act_gid)."' AND fid = '".$val->id."' AND ngg_type = '".NGGCF_GALLERY."'");
				switch($val->field_type) {
				case NGGCF_FIELD_TYPE_INPUT :
					echo '<input name="nggcf_gallery['.$val->id.']" style="width:95%;" value="'.esc_attr($value->field_value).'" />';
					break;
				case NGGCF_FIELD_TYPE_TEXTAREA :
					echo '<textarea name="nggcf_gallery['.$val->id.']" style="width:95%;">'.esc_attr($value->field_value).'</textarea>';
					break;
				case NGGCF_FIELD_TYPE_SELECT :
					$opts = explode(",", $val->drop_options);
					echo '<select name="nggcf_gallery['.$val->id.']">';
					echo '<option value=""></option>';
					foreach ((array)$opts as $optVal) {
						echo '<option value="'.trim(esc_attr($optVal)).'" '.(trim(esc_attr($optVal)) == esc_attr($value->field_value) ? ' selected="selected"' : '').'>'.trim(esc_attr($optVal)).'</option>';
					}
					echo '</select>';
					break;
				}
	...
	//front end stuff{
		...
		function nggcf_get_field($pid, $fname) {
			...
			return $nggcf_values[$pid][$fname]->field_value;
		}
		...
		function nggcf_image_obj($pictureObj, $pid) {
			...
			foreach ($nggcf_values[$pid] as $key=>$val) {
				@$pictureObj->ngg_custom_fields[$key] = $val->field_value;
			}
